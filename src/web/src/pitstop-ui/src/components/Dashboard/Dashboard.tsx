...
const VehicleHeroCard: React.FC<VehicleHeroCardProps> = ({
  vehicles,
  selectedVehicle,
  selectedVehicleId,
  onSelectVehicle,
  ...
  <Card>
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] gap-6 items-center">
        ...
        <div className="min-w-0 flex-1">
          {vehicles.length > 1 ? (
            <Dropdown
              ...
              onChange={(e) => onSelectVehicle(e.value)}
              className="w-full"
            />
          ) : (
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <KpiTile icon={faGaugeHigh} label="Avg MPG" value={avgMpg === null ? "—" : avgMpg.toFixed(1)} />
        <KpiTile icon={faDollarSign} label="$ / mile" value={costPerMile === null ? "—" : costPerMile.toFixed(3)} />
        <KpiTile icon={faGasPump} label="Last fill" value={lastCost === null ? "—" : `$${lastCost.toFixed(2)}`} />
        <KpiTile
          icon={faRoad}
          label="Est. range"
...
interface MpgTrendCardProps {
  mpgChartData: ReturnType<typeof buildMpgChartData>;
}

const MpgTrendCard: React.FC<MpgTrendCardProps> = ({ mpgChartData }) => (
  <Card title="MPG Trend" className="lg:col-span-2">
    {mpgChartData.labels.length > 1 ? (
      <div className="h-64">
        <Chart type="line" data={mpgChartData} options={CHART_OPTIONS} className="h-full w-full" />
      </div>
    ) : (
...
interface RecentFillUpsCardProps {
  recentList: FillUp[];
  onAddFillUp: () => void;
}

const RecentFillUpsCard: React.FC<RecentFillUpsCardProps> = ({ recentList, onAddFillUp }) => (
  <Card title="Recent Fill-Ups">
    {recentList.length === 0 ? (
      <div className="text-center py-8 text-content-muted">
        <FontAwesomeIcon icon={faGasPump} className="text-3xl mb-3" />
        <p>No fill-ups recorded yet.</p>
        <Button label="Add your first fill-up" className="p-button-text mt-2" onClick={onAddFillUp} />
      </div>
    ) : (
      <ul className="divide-y divide-border">
        {recentList.map((f) => (
          <li key={f.id} className="py-3 flex justify-between items-center gap-3">
            <div className="min-w-0">
              <p className="font-medium truncate">{new Date(f.filledAt ?? 0).toLocaleDateString()}</p>
              <p className="text-sm text-content-muted truncate">
                {(num(f.gallonsAdded) ?? 0).toFixed(2)} gal · {(num(f.odometerReading) ?? 0).toLocaleString()} mi
              </p>
            </div>
            <div className="text-right shrink-0">
              <p className="font-numeric font-semibold text-brand">${(num(f.totalCost) ?? 0).toFixed(2)}</p>
              <p className="text-sm text-content-muted">${(num(f.pricePerGallon) ?? 0).toFixed(3)}/gal</p>
            </div>
          </li>
        ))}
      </ul>
...
interface SpendCardProps {
  spendChartData: ReturnType<typeof buildSpendChartData>;
}

const SpendCard: React.FC<SpendCardProps> = ({ spendChartData }) => (
  <Card title="Recent Spend">
    {spendChartData.labels.length > 0 ? (
      <div className="h-64">
        <Chart type="bar" data={spendChartData} options={CHART_OPTIONS} className="h-full w-full" />
      </div>
    ) : (
...
function buildMpgChartData(sortedFillUps: FillUp[]) {
  const withMpg = sortedFillUps.filter((f) => num(f.mpgThisFillUp) !== null);
  return {
    labels: withMpg.map((f) =>
      new Date(f.filledAt ?? 0).toLocaleDateString(undefined, { month: "short", day: "numeric" }),
    ),
    datasets: [
      {
        label: "MPG",
...
function buildSpendChartData(sortedFillUps: FillUp[]) {
...
export const Dashboard: React.FC = () => {
  ...
  const sortedFillUps = useMemo(
    () => [...recentFillUps].sort((a, b) => new Date(a.filledAt ?? 0).getTime() - new Date(b.filledAt ?? 0).getTime()),
    [recentFillUps],
  );
  ...
  const avgMpg = useMemo(() => {
    const vals = sortedFillUps.map((f) => num(f.mpgThisFillUp)).filter((v): v is number => v !== null);
  ...
  const mpgChartData = useMemo(() => buildMpgChartData(sortedFillUps), [sortedFillUps]);
  const spendChartData = useMemo(() => buildSpendChartData(sortedFillUps), [sortedFillUps]);
      ...
      <div className="space-y-6">
        <Card>
          <div className="text-center py-12">
            <FontAwesomeIcon icon={faCar} className="text-4xl mb-3 text-content-muted" />
  ...
  return (
    <div className="space-y-6">
      ...
      <VehicleHeroCard
        ...
        onSelectVehicle={(id) => dispatch(selectVehicle(id))}
        avgMpg={avgMpg}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <MpgTrendCard mpgChartData={mpgChartData} />
        <RecentFillUpsCard recentList={recentList} onAddFillUp={() => navigate("/fill-ups/new")} />
      </div>

      <SpendCard spendChartData={spendChartData} />
    </div>
  );
};
